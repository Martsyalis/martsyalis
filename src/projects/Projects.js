...
export default class Projects extends PureComponent {
  ...
  componentDidUpdate(prevProps, prevState){
    if(prevState.selection !== this.state.selection){
      ...
      projects.map(project=>{
        if(!project.tech.includes(prevState.selection) && project.tech.includes(this.state.selection)){
          newTitles += ` ${project.title} `;
        }
        if(!project.tech.includes(this.state.selection)&& project.tech.includes(prevState.selection)){
  ...
  handleDelay = (title)=>{
    ...
    if(this.state.newTitles.includes(title)) return 'display-project-without-delay';
  ...
  render() {
    const projectsComponents = projects.map((project, i) => (
      <Project
        className={`${
          this.isSelected(project) ? `${this.handleDelay(project.title)}` : 'hide-project'
        }`}
        project={project}
        key={i}
      />
